...
main() {
    ...
    if [[ $highest_level -ge 3 ]]; then
        ...
        # SMS si configuré
        # SECURITE: utiliser un appel direct plutôt que eval pour éviter
        # toute injection de commande si SMS_COMMAND provient d'une source externe.
        if [[ -n "$SMS_COMMAND" && -x "$SMS_COMMAND" ]]; then
            "$SMS_COMMAND" "[NOVA URGENT] Disque >95% sur $HOSTNAME — Intervenir immédiatement"
        elif [[ -n "$SMS_COMMAND" ]]; then
            log "WARN" "SMS_COMMAND configuré mais non exécutable: $SMS_COMMAND — alerte SMS ignorée"
        fi
