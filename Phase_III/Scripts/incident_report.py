...
def main():
    ...
    if args.alert:
        ...
        try:
            # SECURITE: Limiter la taille de l'entrée stdin pour éviter
            # une saturation mémoire (OOM) par un payload JSON malveillant.
            MAX_STDIN_BYTES = 1_048_576  # 1 MB maximum
            raw_alert = json.loads(sys.stdin.read(MAX_STDIN_BYTES))
