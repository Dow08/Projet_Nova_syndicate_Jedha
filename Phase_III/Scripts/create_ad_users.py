...
def main():
    ...
    # SECURITE: Les credentials sont chiffrés avec Fernet (AES-128-CBC + HMAC-SHA256)
    # avant d'être écrits sur disque. La clé est dérivée de VAULT_SECRET (variable d'env).
    # Ne jamais stocker les mots de passe en clair sur le disque.
    if not args.dry_run and results["created"]:
        import os
        import base64
        vault_secret = os.getenv("VAULT_SECRET", "")
        if not vault_secret:
            log.error("VAULT_SECRET non défini — impossible de chiffrer les credentials.")
            log.error("Définissez la variable d'environnement VAULT_SECRET avant de relancer.")
            log.error("Exemple: export VAULT_SECRET=$(openssl rand -hex 32)")
            # On ne sauvegarde PAS les credentials en clair si le secret n'est pas défini
            log.warning("Les mots de passe temporaires n'ont PAS été sauvegardés sur disque.")
            log.warning("Consultez les logs ci-dessus pour les credentials générés en mémoire.")
        else:
            try:
                from cryptography.fernet import Fernet
                import hashlib
                # Dériver une clé Fernet 32 octets depuis le secret (HKDF simplifié)
                key = base64.urlsafe_b64encode(
                    hashlib.sha256(vault_secret.encode()).digest()
                )
                fernet = Fernet(key)
                creds_file = f"credentials_enc_{datetime.now().strftime('%Y%m%d_%H%M%S')}.dat"
                # Chiffrer le contenu CSV en mémoire
                import io
                buf = io.StringIO()
                writer = csv.DictWriter(buf, fieldnames=["username", "email", "profil", "password_temp", "status"])
                writer.writeheader()
                writer.writerows(results["created"])
                encrypted = fernet.encrypt(buf.getvalue().encode())
                with open(creds_file, 'wb') as f:
                    f.write(encrypted)
                os.chmod(creds_file, 0o600)  # Lecture/écriture propriétaire uniquement
                log.info(f"Credentials chiffrés sauvegardés dans {creds_file} (mode 600)")
                log.warning("   → Décrypter avec: python -c \"from cryptography.fernet import Fernet; "
                            "import base64, hashlib; key=base64.urlsafe_b64encode("
                            "hashlib.sha256(open('secret.txt').read().strip().encode()).digest()); "
                            "print(Fernet(key).decrypt(open('credentials_enc_*.dat','rb').read()).decode())\"")
                log.warning("   → Supprimer ce fichier après transfert dans le coffre-fort !")
            except ImportError:
                log.error("Module 'cryptography' manquant. Installer: pip install cryptography")
                log.error("Les credentials NE SONT PAS sauvegardés sur disque par mesure de sécurité.")
